...
if DO_MiddleBurryOther:
    ...
    for dir in subdir:
        ...
        frames_to_interpolate = sorted(os.listdir(os.path.join(MB_Other_DATA, dir)))
        if (len(frames_to_interpolate) > 1):
            arguments_strFirst = os.path.join(MB_Other_DATA, dir, frames_to_interpolate[0])
            arguments_strSecond = os.path.join(MB_Other_DATA, dir, frames_to_interpolate[1])
            arguments_strOut = os.path.join(gen_dir, dir, 'frame'+str(int(frames_to_interpolate[0].split('frame')[1])+1))
