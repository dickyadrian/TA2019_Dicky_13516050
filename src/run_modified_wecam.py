...
if __name__ == '__main__':
	...
	# video = cv2.VideoWriter('record.mp4', fourcc, fps, (frame1.shape[1], frame1.shape[0]))
	time.sleep(5)
	success1, frame1 = vidcap.read()
	...
	print('Start loop')
	while True:
		tensorFirst = torch.FloatTensor(frame1[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))
		tensorSecond = torch.FloatTensor(frame2[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))
