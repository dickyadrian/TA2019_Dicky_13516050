...
if __name__ == '__main__':
	INTERPOLATED_PATH = 'inter_result'
	DATA_PATH = 'VideoData'
	if not os.path.exists(INTERPOLATED_PATH):
		os.mkdir(INTERPOLATED_PATH)
	if True:
		for subdir in os.listdir(DATA_PATH):
			inputs = sorted(os.listdir(os.path.join(DATA_PATH, subdir)))
			if (len(inputs) > 1):
				first = os.path.join(DATA_PATH, subdir, inputs[0])
				second = os.path.join(DATA_PATH, subdir, inputs[1])
				out = os.path.join(INTERPOLATED_PATH, 'frame'+str(int(inputs[0].split('frame').replace('.jpg', '')) + 1)+'.jpg')
				...
				PIL.Image.fromarray((tensorOutput.clamp(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, ::-1] * 255.0).astype(numpy.uint8)).save(os.path.join(INTERPOLATED_PATH, out))
